...
1. `mcp__gray-matter__pre_turn(topic, keywords)` BEFORE replying on any
   substantive turn. Fold the returned context silently into the reply —
   never announce the call or quote its raw output.
2. Reply to the user.
3. `mcp__gray-matter__store_turn(topic, keywords, links, domain, intent,
...
the graph is empty. Call `mcp__gray-matter__help` once per session if unsure of
the command list.
...
- **Before minting a concept that may already exist**: check with
  `mcp__gray-matter__vector_search(query)` or `mcp__gray-matter__find_candidates` and
...
- `mcp__gray-matter__confirm(keywords)` when context returned by pre_turn was
  actually useful — it boosts salience of the right nodes.
- `mcp__gray-matter__dedup` / `mcp__gray-matter__merge` for duplicates;
  `mcp__gray-matter__consolidate` for periodic cleanup (do NOT run consolidate
  ad-hoc on a SHARED cloud store — coordinate with the team first).
- `mcp__gray-matter__switch_context(path)` to work in a separate context
...
Call `mcp__gray-matter__skill` with `name='playbook'` for the complete
playbook served by the server itself, or `mcp__gray-matter__help` for the
